...
typedef enum { HC_IDLE = 0, HC_XFRC, HC_HALTED, HC_NAK, HC_NYET, HC_STALL, HC_XACTERR, HC_BBLERR, HC_DATATGLERR } HC_STATUS;
...
typedef struct _Device_cntrl {
    __ALIGN_BEGIN uint8_t pbuf[64]; __ALIGN_END
    uint8_t     Req;
    uint8_t     Cmd;
    uint32_t    Len;
    uint8_t     Unit;
} USBD_Class_CntrlReq_TypeDef;

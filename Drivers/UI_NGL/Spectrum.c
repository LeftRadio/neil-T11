...
void Spectrum_Cacl_and_Render(void)
{
	...
	if((AudioBuffer_HalfFull != RESET) && (update == 1))
	{
		...
		do
		{
			LeftData = (int16_t)(((int16_t)AudioBuffer[i + 1] << 8) | (int16_t)AudioBuffer[i]);
			RightData = (int16_t)(((int16_t)AudioBuffer[i + 3] << 8) | (int16_t)AudioBuffer[i + 2]);
...
static void SpectrumRender(int16_t* Bands_buf, uint16_t Lenght)
{
	...
	uint16_t i, j, k;
	float Y;
	float FrqY = (float)AudioInterface.I2S->I2S_AudioFreq / (float)(1 << NS_P);
	uint16_t index;
	long sum = 0;
